...
CREATE TABLE user_profiles (
    ...
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id_str) REFERENCES users_xp(user_id_str)
);
...
CREATE INDEX idx_users_xp ON users_xp(xp DESC);
CREATE INDEX idx_users_msg_alltime_desc ON users_xp (msg_alltime DESC);
CREATE INDEX idx_users_vc_alltime_desc ON users_xp (vc_alltime DESC);
CREATE INDEX idx_users_guild_user ON users_xp (guild_id, user_id_str);
CREATE INDEX idx_users_last_msg ON users_xp (last_msg_at DESC);
...
CREATE INDEX idx_user_levels_user ON users_xp (user_id_str);
CREATE INDEX idx_user_levels_level ON users_xp (level);
CREATE INDEX idx_user_levels_xp_total ON users_xp (xp DESC);
...
CREATE TRIGGER update_users_from_profile
    ...
    BEGIN
        UPDATE users_xp SET updated_at = CURRENT_TIMESTAMP
        WHERE user_id_str = NEW.user_id_str;
    END;
...
-- Update timestamps
UPDATE users_xp SET updated_at = CURRENT_TIMESTAMP WHERE updated_at IS NULL;
